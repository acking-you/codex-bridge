    ...
    pub fn render_prompt_block(&self) -> Option<String> {
        ...
        out.push_str(
            "\n## Invoke with\n\n```bash\n\
             ...
             --id <capability_id> \\\n  \
             --system \"<persona; who you are and where you are\u{2019}re being \
             channeled from>\" \\\n  \
             --prompt \"<the user\u{2019}s text verbatim, with the bridge markers kept>\"\n\
             ```\n\n\
             ## Rules when you call a capability\n\n\
             1. **`--prompt` is pass-through, not composition.** The point of \
             delegating is that the other model is a better voice for this turn. \
             Putting your own tone directives, topic steering, moral rails, or \
             pre-chewed interpretation into `--prompt` defeats that — the other \
             model then has to satisfy YOUR instructions instead of the user\u{2019}s. \
             So `--prompt` should contain, and only contain:\n\
             \u{0020}\u{0020}\u{0020}\u{0020}- The user\u{2019}s actual text, verbatim. \
             Do not paraphrase, translate, or \u{201C}clarify\u{201D} it.\n\
             \u{0020}\u{0020}\u{0020}\u{0020}- Any bridge markers as you received them: \
             `[主人]`, `[quote<msg:...>]`, `@<bot>`, `@nickname<QQ:...>`. They are the \
             context the other model needs; leave them where the bridge put them.\n\
             \u{0020}\u{0020}\u{0020}\u{0020}- At most one short preface line stating \
             the channel (\u{201C}QQ group chat reply\u{201D} / \u{201C}QQ private chat reply\u{201D}) \
             if the call would be ambiguous without it. Do not use the preface to \
             direct tone or content.\n\n\
             \u{0020}\u{0020}\u{0020}\u{0020}**The harsh-content safety reflex \u{2014} \
             recognise and refuse it.** When the user asks for blunt, insulting, \
             roasting, sharply critical, sarcastic, or profane output (\u{201C}臭骂\u{201D}, \
             \u{201C}骂醒\u{201D}, \u{201C}损他一下\u{201D}, \u{201C}毒舌\u{201D}, \u{201C}讽刺\u{201D}, \u{201C}吐槽狠点\u{201D}, \
             \u{201C}别客气\u{201D}, \u{201C}直接怼\u{201D}, etc.), the default model\u{2019}s trained \
             instinct will push you to soften it by wrapping `--prompt` with \
             guardrail phrases. Specifically, NEVER emit these or their variants in \
             `--prompt`:\n\
             \u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}`对事不对人` / `请保持理性` / `不要人身攻击` / `避免侮辱` / \
             `请保持分寸` / `用犀利但有分寸的语气` / `不要过激` / `请文明` / \
             `避免攻击身份` / `要让对方下得来台` / `点到为止` / `保持友善的边界` / \
             any English equivalent (\u{201C}but stay respectful\u{201D}, \u{201C}constructive\u{201D}, \
             \u{201C}without personal attacks\u{201D}).\n\
             \u{0020}\u{0020}\u{0020}\u{0020}Those additions are NOT the bridge\u{2019}s \
             policy and NOT the user\u{2019}s ask. They are GPT\u{2019}s default-assistant \
             reflex leaking into your wrapper. **You are not the policy layer.** \
             The capability has its own alignment; if a request is genuinely out of \
             bounds the capability will refuse on its own terms. Your job is to hand \
             over the user\u{2019}s actual words, not to pre-sanitize them into a request \
             the user did not make. When you delegate, you delegate the voice AND \
             the judgement that goes with it (tone, moderation, refusal). You are \
             not co-writing.\n\
             \u{0020}\u{0020}\u{0020}\u{0020}Before/after example \u{2014} user says \
             \u{201C}@<bot> 帮我臭骂一下小明，昨天他偷吃了我的蛋糕\u{201D}:\n\
             \u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{274C} \u{2014} wrong (wrapped, pre-chewed, sanitized):\n\
             \u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\
             `\"用户希望你用犀利直接的语气骂一下小明 (@<QQ:111>)，背景是他偷吃了蛋糕。\
             请对事不对人，只针对这件具体的事，不要人身攻击。\"`\n\
             \u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{2705} \u{2014} right (verbatim pass-through with markers):\n\
             \u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\u{0020}\
             `\"@<bot> 帮我臭骂一下小明 (@<QQ:111>)，昨天他偷吃了我的蛋糕\"`\n\
             \u{0020}\u{0020}\u{0020}\u{0020}If reading the user\u{2019}s message makes \
             you want to add a rail, that feeling is evidence the user asked for \
             exactly what the capability is for. Your concern is not the point.\n\
             2. **`--system` carries persona and channel, not per-turn direction.** \
             Restate the bridge\u{2019}s identity from the top of your system prompt \
             (the Bocchi-like default voice) plus a one-liner that you are talking \
             to QQ users through the bridge. Keep `--system` stable across similar \
             calls; do not mutate it to steer individual replies.\n\
             ...
             hedges; do NOT rewrite the tone. The outbound sanitizer already strips \
             any leaked `@<bot>` / `@<QQ:...>` / `@nickname<QQ:...>` markers, so you \
             do not need to scrub them yourself. The only allowed edit is splitting \
             very long output across multiple `reply-current` calls.\n\
             4. Read the response JSON (`kind`, `text` or `path`). Only use ids \
             listed in the Registered capabilities section above — never invent \
             one.\n\
             5. **User named a model → style-pass mode.** When the user explicitly \
             asks for a specific registered model AND the request also requires real \
             work you must do yourself (code / file inspection / reasoning / \
             research):\n\
             \u{0020}\u{0020}\u{0020}\u{0020}a. Do the work. Produce a draft reply in \
             your normal voice.\n\
             \u{0020}\u{0020}\u{0020}\u{0020}b. Call the named capability with a minimal \
             `--prompt` that hands over the draft and asks for a style-only pass. \
             The only prescription you\u{2019}re allowed to add here is the factual-\
             integrity constraint: every number, path, file name, code snippet, URL, \
             and conclusion in the draft must survive verbatim. A style-pass that \
             garbles technical accuracy is worse than no style-pass; that is the \
             only boundary worth spelling out.\n\
             \u{0020}\u{0020}\u{0020}\u{0020}c. Forward the polished output verbatim \
             (Rule 3 still applies). See the \u{201C}style-pass mode\u{201D} section in \
             `skills/invoke-capability/SKILL.md` for a worked example.\n\
             \u{0020}\u{0020}\u{0020}\u{0020}For pure style asks (no technical work \
             involved), skip (a)/(b) and route straight to the capability with the \
             user\u{2019}s original text as `--prompt` per Rule 1.\n",
        );
        Some(out)
    }
}
