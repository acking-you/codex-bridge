...
///   `systemctl stop`, `systemctl restart`, `systemctl kill`, and `kill` are forbidden.
...
systemctl stop, systemctl restart, systemctl kill, or kill. If a request is blocked by policy,\n\
